...
module.exports = function registerMock(app) {
  app.post('/api/mock/v1/chat/completions', (req, res) => {
    ...
    if (body.stream !== true) return res.json({ id: `mock-${Date.now()}`, model: body.model || 'echo', choices: [{ index: 0, message: { role: 'assistant', content: output }, finish_reason: 'stop' }], usage });
    res.setHeader('Content-Type', 'text/event-stream');
    res.setHeader('Cache-Control', 'no-cache');
    const chunks = output.match(/.{1,8}/gu) || [output];
    for (const content of chunks) res.write(`data: ${JSON.stringify({ id: `mock-${Date.now()}`, model: body.model || 'echo', choices: [{ index: 0, delta: { content }, finish_reason: null }] })}\n\n`);
