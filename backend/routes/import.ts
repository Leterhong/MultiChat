...
function normalizeAgent(raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('agent must be an object');
  ...
  if (!name || name.length > 120) throw new Error('agent name must be 1-120 characters');
  if (systemPrompt.length > 30_000) throw new Error('systemPrompt exceeds 30000 characters');
...
module.exports = function registerImport(app) {
  app.post('/api/import', async (req, res) => {
    ...
    if (typeof body.payload === 'string') {
      if (body.payload.length > 2_000_000) return fail(res, 413, 'IMPORT_FAILED', '导入文件超过 2 MB');
      try { value = JSON.parse(body.payload); }
    ...
    let normalized;
    try { normalized = incoming.map(normalizeAgent); }
    catch (error) { return fail(res, 400, 'INVALID_PACKAGE', error.message); }
    await ctx.store.mutate(ctx.AGENT_FILE, agents => {
      for (const item of normalized) {
        const index = agents.findIndex(agent => agent.id === item.id);
        if (index >= 0) agents[index] = item;
