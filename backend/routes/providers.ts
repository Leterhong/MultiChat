...
function normalizeProvider(input: JsonRecord, previous: JsonRecord | null = null) {
  ...
  if (!ctx.ADAPTER_MAP[apiType]) throw new Error('unsupported provider apiType');
  const baseUrl = String(input.baseUrl ?? previous?.baseUrl ?? '').trim().replace(/\/+$/, '');
  if (!baseUrl || baseUrl.length > 2048) throw new Error('baseUrl is required');
  let parsed: URL;
  try { parsed = new URL(baseUrl); } catch { throw new Error('baseUrl must be a valid URL'); }
  ...
  const models: string[] = Array.isArray(modelInput)
    ? [...new Set(modelInput.map(value => String(value).trim()).filter(Boolean))].slice(0, 200)
    : [];
  if (models.some(model => model.length > 200)) throw new Error('model name is too long');
  return {
    id,
...
module.exports = function registerProviders(app) {
  ...
  app.post('/api/providers', (req, res) => {
    let provider;
    try { provider = normalizeProvider(req.body || {}); }
    catch (error) { return res.status(400).json({ error: error.message }); }
    const providers = ctx.providerStore.list();
    if (providers.some(item => item.id === provider.id)) return res.status(409).json({ error: 'provider id already exists' });
  ...
  app.put('/api/providers/:id', (req, res) => {
    ...
    let updated;
    try { updated = normalizeProvider({ ...body, apiKey }, prev); }
    catch (error) { return res.status(400).json({ error: error.message }); }
  ...
  app.post('/api/providers/:id/probe', async (req, res) => {
    const providers = ctx.providerStore.list();
    const idx = providers.findIndex(p => p.id === req.params.id);
    if (idx < 0) return res.status(404).json({ error: 'provider not found', code: 'NOT_FOUND' });
    const provider = providers[idx];
    const baseUrl = String(provider.baseUrl || '').replace(/\/+$/, '');
    let parsed: URL;
    try { parsed = new URL(baseUrl); } catch { return res.status(400).json({ error: 'provider baseUrl 无效' }); }
    ...
    try {
      ...
      const data = await readResponseJson(resp, 2_000_000) as JsonRecord;
      const raw = Array.isArray(data.data) ? data.data : Array.isArray(data.models) ? data.models : [];
      const models = raw.map((m: JsonRecord) => String(m?.id || m?.name || '')).filter(Boolean);
      const capabilities = {
      ...
      try {
        providers[idx] = { ...provider, capabilities: { ...(provider.capabilities as JsonRecord | undefined), ...capabilities } };
        ctx.providerStore.save(providers);
  ...
  app.post('/api/fetch-local-models', async (req, res) => {
    ...
    try {
      let parsed: URL;
      try { parsed = new URL(baseUrl); } catch { return res.status(400).json({ error: 'baseUrl must be a valid URL' }); }
