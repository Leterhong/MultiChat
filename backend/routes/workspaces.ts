...
module.exports = function registerWorkspaces(app) {
  ...
  app.post('/api/workspaces', (req, res) => {
    try { res.status(201).json(ctx.workspaceStore.createWorkspace(req.body)); }
    catch (error) { sendWorkspaceError(res, error); }
  });

  app.put('/api/workspaces/:id', (req, res) => {
    try { res.json(ctx.workspaceStore.updateWorkspace(req.params.id, req.body)); }
    catch (error) { sendWorkspaceError(res, error); }
  });

  app.delete('/api/workspaces/:id', (req, res) => {
    try { res.json({ ok: true, workspace: ctx.workspaceStore.removeWorkspace(req.params.id) }); }
    catch (error) { sendWorkspaceError(res, error); }
  });

  app.get('/api/projects', (req, res) => {
    let projects = ctx.workspaceStore.projects();
    if (req.query.workspaceId) projects = projects.filter(project => project.workspaceId === req.query.workspaceId);
    res.json(projects);
  });
  ...
  app.post('/api/projects', (req, res) => {
    try {
      ...
      if (workspace && !workspace.defaultProjectId) ctx.workspaceStore.updateWorkspace(workspace.id, { defaultProjectId: project.id });
      res.status(201).json(project);
    } catch (error) { sendWorkspaceError(res, error); }
  });

  app.put('/api/projects/:id', (req, res) => {
    try { res.json(ctx.workspaceStore.updateProject(req.params.id, req.body)); }
    catch (error) { sendWorkspaceError(res, error); }
  });

  app.delete('/api/projects/:id', (req, res) => {
    try { res.json({ ok: true, project: ctx.workspaceStore.removeProject(req.params.id) }); }
    catch (error) { sendWorkspaceError(res, error); }
  });

  app.get('/api/assets', (req, res) => {
    let assets = ctx.workspaceStore.assets();
    if (req.query.projectId) assets = assets.filter(asset => asset.projectId === req.query.projectId);
    if (req.query.workspaceId) assets = assets.filter(asset => asset.workspaceId === req.query.workspaceId);
  ...
  app.post('/api/assets', async (req, res) => {
    ...
    try {
      ...
      if (body.url) {
        ...
        if (!name) name = value.split('/').pop().split('?')[0] || '远程文件';
      }
  ...
  app.delete('/api/assets/:id', (req, res) => {
    try { res.json({ ok: true, asset: assetMeta(ctx.workspaceStore.removeAsset(req.params.id)) }); }
    catch (error) { sendWorkspaceError(res, error); }
  });
