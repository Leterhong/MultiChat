...
module.exports = function registerMeta(app) {
  app.get('/api/runtime', (req, res) => {
    ...
    res.json({
      ...
      capabilities: ['chat', 'streaming', 'agents', 'agent-skills', 'plugins', 'mcp-discovery', 'codex-sync', 'git-diff', 'run-history'],
  ...
  app.get('/api/catalog', (req, res) => {
    ...
    const plugins = extensions.listPlugins();
    const mcpServers = extensions.listMcpServers();
    res.json({ providers, skills, tools, agents, plugins, mcpServers, workspaces: ctx.workspaceStore.workspaces(), projects: ctx.workspaceStore.projects(), assets: ctx.workspaceStore.assets().map(assetMeta) });
  });
