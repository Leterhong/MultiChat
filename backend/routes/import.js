...
const sp = require('../lib/skillpack');
...
module.exports = function registerImport(app) {
  app.post('/api/import', async (req, res) => {
    ...
    try {
      // ── 技能压缩包（.zip 上传，base64 payload） ──
      if (body.format === 'zip') {
        if (!body.payload) return fail(res, 400, 'BAD_REQUEST', 'zip 上传需提供 body.payload（base64 编码的 .zip）');
        let buf;
        try { buf = Buffer.from(String(body.payload), 'base64'); }
        catch { return fail(res, 400, 'INVALID_PACKAGE', 'payload 不是合法 base64'); }
        let r;
        // 在并发锁内执行解压+落盘+注册，避免多请求同时写 plugins/ 与 skills.json 覆盖
        await ctx.store.mutate('_plugin_ops', () => { r = sp.installSkillPackFromBuffer(buf, { source: tag }); });
        return res.json({ ok: true, plugin: r.plugin, skills: r.skills, agents: r.agents });
      }
      ...
      if (body.url) {
        ...
        try {
          ...
          const resp = await ctx.safeFetch(u, { signal: ctrl.signal, headers: { accept: 'application/json, application/zip' } }, sameOrigin);
          if (!resp.ok) throw new Error('HTTP ' + resp.status);
          const contentType = resp.headers.get('content-type') || '';
          const buf = Buffer.from(await resp.arrayBuffer());
          if (buf.length > 2_000_000) throw new Error('响应超过 2 MB 限制');
          // 压缩包识别：magic PK\x03\x04 / PK\x05\x06，或 content-type / URL 后缀为 zip
          const looksZip = (buf[0] === 0x50 && buf[1] === 0x4b && (buf[2] === 0x03 || buf[2] === 0x05))
            || /application\/zip/i.test(contentType) || /\.zip($|\?)/i.test(u);
          if (looksZip) {
            clearTimeout(timer);
            let r;
            await ctx.store.mutate('_plugin_ops', () => { r = sp.installSkillPackFromBuffer(buf, { source: tag, remote: true }); });
            return res.json({ ok: true, plugin: r.plugin, skills: r.skills, agents: r.agents });
          }
          const text = buf.toString('utf-8');
          if (!/application\/json/i.test(contentType) && !/^\s*[[{]/.test(text)) {
            throw new Error('响应不是 JSON（' + contentType + '）');
          }
      ...
      // 插件清单 / 技能包校验类错误由 AppError 抛出，统一转为结构化失败响应
