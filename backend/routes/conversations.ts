...
function assertSafeId(res, id) {
  if (typeof id === 'string' && SAFE_ID_RE.test(id)) return true;
...
function validateConversationLocation(workspaceId, projectId) {
  ...
  if (workspace && project && project.workspaceId !== workspace.id) throw new Error('project does not belong to workspace');
}

function validateMessages(value) {
  if (!Array.isArray(value)) throw new Error('messages must be an array');
  if (value.length > 2_000) throw new Error('messages exceed 2000 entries');
  if (Buffer.byteLength(JSON.stringify(value), 'utf8') > 16_000_000) throw new Error('messages exceed 16 MB');
  for (const message of value) {
    if (!message || typeof message !== 'object' || Array.isArray(message)) throw new Error('each message must be an object');
...
module.exports = function registerConversations(app) {
  ...
  app.get('/api/conversations/:id', (req, res) => {
    if (!assertSafeId(res, req.params.id)) return;
