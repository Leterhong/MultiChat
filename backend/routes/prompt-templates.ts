...
function normalize(raw) {
  ...
  if (!name || name.length > 80) throw new Error('模板名称必须是 1-80 个字符');
  if (!content || content.length > 30_000) throw new Error('模板内容必须是 1-30000 个字符');
...
module.exports = function registerPromptTemplates(app) {
  ...
  app.post('/api/prompt-templates', (req, res, next) => {
    let tpl;
    try { tpl = normalize(req.body); } catch (e) { return res.status(400).json({ error: e.message, code: 'INVALID_TEMPLATE' }); }
    void ctx.store.mutate('prompt_templates.json', rows => {
        const i = rows.findIndex(r => r.name === tpl.name);
        if (i >= 0) rows[i] = tpl; else rows.push(tpl);
        return rows;
      }, [])
  ...
  app.delete('/api/prompt-templates/:id', (req, res, next) => {
    ...
    void ctx.store.mutate('prompt_templates.json', rows => rows.filter(row => {
        if (row.id === req.params.id) found = true;
