...
function tokenFields(raw: JsonRecord = {}) {
...
interface UsageLedger { version: number; records: JsonRecord[]; }

function ledger(store: JsonStore): UsageLedger {
  const value = store.read<JsonRecord | JsonRecord[]>(USAGE_FILE, { version: SCHEMA_VERSION, records: [] });
  if (Array.isArray(value)) return { version: SCHEMA_VERSION, records: value };
...
export async function recordUsage(store: JsonStore, input: JsonRecord) {
  ...
  await store.mutate<UsageLedger | JsonRecord[]>(USAGE_FILE, current => {
...
function localParts(timestamp: string, offsetMinutes: number) {
  const parsed = Date.parse(timestamp);
  if (!Number.isFinite(parsed)) return { date: '0000-00-00', hour: 0 };
...
function dateRange(days: number, offsetMinutes: number) {
...
function summarizeRows(rows: JsonRecord[], days: number, offsetMinutes: number) {
  const dailyMap = new Map<string, JsonRecord>();
  const modelMap = new Map<string, JsonRecord>();
  const providerMap = new Map<string, JsonRecord>();
  ...
  for (const row of rows) {
    ...
    daily.requests += 1;
    if (row.status !== 'success') daily.errors += 1;
    ...
    model.totalTokens += finite(row.totalTokens); model.inputTokens += finite(row.inputTokens); model.outputTokens += finite(row.outputTokens); model.requests += 1;
    ...
    provider.totalTokens += finite(row.totalTokens); provider.requests += 1; if (row.status !== 'success') provider.errors += 1;
    providerMap.set(providerKey, provider);

    inputTokens += finite(row.inputTokens); outputTokens += finite(row.outputTokens); totalTokens += finite(row.totalTokens);
    cachedTokens += finite(row.cachedTokens); reasoningTokens += finite(row.reasoningTokens);
    if (row.source === 'reported') reportedTokens += finite(row.totalTokens); else estimatedTokens += finite(row.totalTokens);
    if (row.status !== 'success') errors += 1;
    if (row.durationMs) { durationMs += finite(row.durationMs); durationCount += 1; }
  ...
  let currentStreak = 0;
  for (let cursor = new Date(`${today}T00:00:00.000Z`); activeDates.has(cursor.toISOString().slice(0, 10)); cursor = new Date(cursor.getTime() - 86_400_000)) currentStreak += 1;
  ...
  for (const date of [...activeDates].sort()) {
    ...
    running = adjacent ? running + 1 : 1; longestStreak = Math.max(longestStreak, running); previous = date;
  }
