...
import type { AssetRecord, JsonRecord, JsonStore, ProjectRecord, WorkspaceRecord } from '../types';
...
function createWorkspaceStore(store: JsonStore) {
  function workspaces(): WorkspaceRecord[] { return store.read(WORKSPACE_FILE, [] as WorkspaceRecord[]); }
  function projects(): ProjectRecord[] { return store.read(PROJECT_FILE, [] as ProjectRecord[]); }
  function assets(): AssetRecord[] { return store.read(ASSET_FILE, [] as AssetRecord[]); }
  ...
  function getWorkspace(id: string) { return workspaces().find(x => x.id === id) || null; }
  function getProject(id: string) { return projects().find(x => x.id === id) || null; }
  function getAsset(id: string) { return assets().find(x => x.id === id) || null; }
  ...
  function createWorkspace(input: JsonRecord = {}) {
    const timestamp = now();
  ...
  function updateWorkspace(id: string, input: JsonRecord = {}) {
    const rows = workspaces();
  ...
  function createProject(input: JsonRecord = {}) {
  ...
  function updateProject(id: string, input: JsonRecord = {}) {
    const rows = projects();
    const index = rows.findIndex(x => x.id === id);
  ...
  function createAsset(input: JsonRecord = {}) {
