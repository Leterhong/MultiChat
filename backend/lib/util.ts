...
function assetMeta<T extends { content?: unknown }>(asset: T | null | undefined) {
  if (!asset) return asset;
  const { content: _content, ...meta } = asset;
  return meta;
}

async function readResponseText(response: Response, maxBytes = 2_000_000): Promise<string> {
...
async function readResponseJson(response: Response, maxBytes = 2_000_000) {
