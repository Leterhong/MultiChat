...
class AnthropicAdapter extends BaseAdapter {
    ...
    buildRequestBody(model, messages, stream, options = {}) {
        ...
        if (system)
            body.system = system;
        for (const key of ['temperature', 'top_p', 'stop_sequences'])
    ...
    transformSSEChunk(line) {
        ...
        if (event.type === 'content_block_start') {
            ...
            if (block.type === 'tool_use')
                return { choices: [{ delta: { tool_calls: [{ index: event.index || 0, id: block.id, type: 'function', function: { name: block.name, arguments: '' } }] } }] };
            return null;
        }
        if (event.type === 'content_block_delta') {
            ...
            if (delta.type === 'input_json_delta' && delta.partial_json)
                return { choices: [{ delta: { tool_calls: [{ index: event.index || 0, function: { arguments: delta.partial_json } }] } }] };
            return null;
        }
