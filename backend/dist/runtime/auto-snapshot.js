...
exports.runAutoSnapshotSweep = runAutoSnapshotSweep;
exports.startAutoSnapshots = startAutoSnapshots;
...
function runAutoSnapshotSweep(nowMs = Date.now()) {
    if (process.env.MULTICHAT_AUTO_SNAPSHOT === '0')
        return { created: [], pruned: 0, disabled: true };
    const created = [];
    let pruned = 0;
    ...
    for (const project of projects) {
        ...
        if (Number.isFinite(newest) && nowMs - newest < DUE_MS)
            continue;
        try {
    ...
    if (created.length) {
        ...
        if (pruned > 0)
            ctx.store.write('snapshots.json', next);
    }
...
function startAutoSnapshots() {
    if (process.env.MULTICHAT_AUTO_SNAPSHOT === '0')
        return;
    const boot = setTimeout(() => { try {
        runAutoSnapshotSweep();
    }
    catch { } }, 2 * 60 * 1000);
    const timer = setInterval(() => { try {
        runAutoSnapshotSweep();
    }
    catch { } }, SWEEP_INTERVAL_MS);
    boot.unref?.();
    timer.unref?.();
}
