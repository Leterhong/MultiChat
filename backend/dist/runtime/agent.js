...
function calculateExpression(input) {
    ...
    if (!source || source.length > 500)
        throw new Error('expression must be 1-500 characters');
    ...
    const expression = () => {
        let value = term();
        while (['+', '-'].includes(peek())) {
            const operator = take(), right = term();
...
function skillPermissions(skill) {
    if (Array.isArray(skill?.config?.permissions))
        return skill.config.permissions.map(String);
    if (skill?.type === 'web_fetch' || skill?.type === 'web_search')
        return ['network'];
    if (skill?.type === 'mcp')
        return ['external'];
    return [];
...
function requiresApproval(skill, policy) {
    ...
    if (!perms.length)
        return false;
    if (skill?.type === 'mcp' && trustLevel(skill) === 'trusted')
        return false;
    return true;
}
...
function collectResourceFiles(skill) {
    ...
    const walk = (kind, current, depth = 0) => {
        if (depth > 3 || !ctx.fs.existsSync(current) || result[kind].length >= 40)
            return;
        for (const entry of ctx.fs.readdirSync(current, { withFileTypes: true })) {
            ...
            if (entry.isDirectory())
                walk(kind, absolute, depth + 1);
            else if (entry.isFile())
                result[kind].push(ctx.path.relative(directory, absolute).replace(/\\/g, '/'));
            if (result[kind].length >= 40)
                break;
        }
    };
    for (const kind of Object.keys(result))
        walk(kind, ctx.path.join(directory, kind));
    return result;
}
function workflowPayload(skill) {
    ...
    for (const relative of files.references) {
        if (remaining <= 0 || !/\.(?:md|mdx|txt|json|ya?ml|csv)$/i.test(relative))
            continue;
        try {
    ...
    try {
        switch (skill.type) {
            ...
            case 'calculator': {
                ...
                try {
                    return wrap(String(calculateExpression(expr)));
                }
                catch (e) {
                    return fail('calculation error: ' + e.message);
                }
            }
            case 'web_fetch': {
                const url = String((args && args.url) || '').trim();
                if (!/^https?:\/\//i.test(url))
                    return fail('only http(s) URL allowed');
            ...
            case 'mcp': {
                ...
                if (!launch || !tool)
                    return fail('mcp skill missing launch config or tool name');
                const client = ctx.getMcpClient(launch);
                if (!client)
                    return fail('cannot start mcp client');
                const r = await client.callTool(tool, args, { signal });
                if (!r)
                    return fail('empty mcp response');
                if (r.isError)
                    return fail(String(r.content && r.content[0] && r.content[0].text || 'mcp error'));
                return wrap({
...
function findSkillForTool(name, skills) {
    for (const s of skills) {
        if (s.type === 'mcp' && s.config && s.config.tool === name)
            return s;
        const sc = SKILL_SCHEMAS[s.type];
        if (sc && sc.name === name)
            return s;
...
function projectSearch(projectId, query, assetIds, limit = 12) {
    ...
    for (const asset of ctx.workspaceStore.assets().filter(item => item.projectId === projectId && (!requested || requested.has(item.id)))) {
        ...
        for (let start = 0; start < lines.length; start += 24) {
            ...
            if (!terms.length)
                score = 1 / (1 + start);
            if (score > 0 && results.length < 2000)
                results.push({
                    assetId: asset.id,
                    name: asset.name,
                    lineStart: start + 1,
...
function buildAssetContext(projectId, assetIds, query = '') {
    ...
    if (totalSize <= 48_000) {
        ...
        parts = projectSearch(projectId, query, assetIds, 14).map(result => `### [Source: ${result.name}#L${result.lineStart}-L${result.lineEnd}]\n${result.snippet}`);
    }
    return parts.length
...
function ensureSeed() {
    let seeded = false;
    if (ctx.store.read(ctx.SKILL_FILE, null) === null) {
        ctx.store.write(ctx.SKILL_FILE, defaultSkills());
        seeded = true;
    }
    if (ctx.store.read(ctx.AGENT_FILE, null) === null) {
        ctx.store.write(ctx.AGENT_FILE, defaultAgents());
        seeded = true;
    }
