...
function isMetadataAddress(ip) {
    ...
    return Boolean(v4 && (v4.startsWith('0.') || v4.startsWith('169.254.') || v4 === '100.100.100.200')) || value === '::' || value === 'fd00:ec2::254' || value.startsWith('fe80:');
}
async function assertSafeUrl(input, { allowPrivate = false } = {}) {
    let url;
    try {
        url = new URL(input);
    }
    ...
    if (net.isIP(host) === 4) {
        if (isMetadataAddress(host))
            throw new Error('SSRF: 目标为保留、元数据或链路本地地址 ' + host);
        if (!allowPrivate && isPrivateV4(host))
            throw new Error('SSRF: 目标为内网地址 ' + host);
    }
        ...
        for (const { address } of addrs) {
            ...
            if (isMetadataAddress(address))
                throw new Error('SSRF: 解析到保留、元数据或链路本地地址 ' + address);
            if (v === 4 && !allowPrivate && isPrivateV4(address))
                throw new Error('SSRF: 解析到内网地址 ' + address);
            if (v === 6 && !allowPrivate && isPrivateV6(address))
                throw new Error('SSRF: 解析到内网地址 ' + address);
        }
    }
    return url;
...
async function safeFetch(input, options = {}, allowPrivate = false, depth = 0) {
