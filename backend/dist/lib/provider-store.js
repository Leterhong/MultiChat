...
function keyFor(store) {
    ...
    if (key.length !== 32)
        throw new Error('provider secret key is invalid');
    protectFile(file);
    return key;
}
function encrypt(store, value) {
...
function createProviderStore(store) {
    function stored() {
        const rows = store.read(PROVIDER_FILE, []);
        let migrated = false;
