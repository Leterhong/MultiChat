...
function createWorkspaceStore(store) {
    ...
    function ensureSeed() {
        ...
        if (!store.read(ASSET_FILE, null))
            store.write(ASSET_FILE, []);
    }
    ...
    function removeProject(id) {
        ...
        if (projects().filter(x => x.workspaceId === project.workspaceId).length <= 1)
            throw new Error('a workspace must keep one project');
    ...
    function createAsset(input = {}) {
        ...
        if (Buffer.byteLength(content, 'utf8') > 2_000_000)
            throw new Error('asset content exceeds 2 MB');
        const timestamp = now();
