...
function createSqliteStore(rootDir) {
  ...
  `);
    ...
    function importJsonFiles() {
        if (migrationValue.get())
            return;
        database.exec('BEGIN IMMEDIATE');
        try {
            const names = fs.readdirSync(rootDir).filter((name) => name.endsWith('.json') && path.basename(name) === name);
            for (const name of names) {
                if (selectValue.get(name))
                    continue;
                try {
    ...
    function read(name, fallback) {
        validName(name);
        const row = selectValue.get(name);
        if (!row?.value)
            return fallback;
        try {
    ...
    function reindex(source, value) {
        if (!['assets.json', 'memories.json'].includes(source))
            return;
    ...
    function mutate(name, update, fallback) {
    ...
    function searchDocuments(query, options = {}) {
      ...
      bm25(search_fts) AS rank FROM search_fts`;
        try {
          ...
          ORDER BY rank LIMIT ?`).all(phrase, projectId, projectId, limit);
            }
        }
        catch {
      ...
      LIMIT ?`).all(like, like, projectId, projectId, limit);
    }
    importJsonFiles();
