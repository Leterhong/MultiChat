...
module.exports = function registerSkills(app) {
    app.get('/api/skills', (req, res, next) => {
        try {
            res.json(extensions.listSkills());
        }
        catch (error) {
            next(error);
        }
    ...
    app.post('/api/skills', (req, res, next) => {
        try {
            res.status(201).json(extensions.createSkill(req.body || {}));
        }
        catch (error) {
            next(error);
        }
    ...
    app.put('/api/skills/:id', (req, res, next) => {
        try {
            ...
            if (body.enabled !== undefined)
                extensions.setSkillEnabled(req.params.id, body.enabled !== false);
            res.json(item);
        }
        catch (error) {
            next(error);
    ...
    app.post('/api/skills/:id/toggle', (req, res, next) => {
        try {
            res.json(extensions.setSkillEnabled(req.params.id, req.body?.enabled !== false));
        }
        catch (error) {
            next(error);
        }
    ...
    app.delete('/api/skills/:id', (req, res, next) => {
        try {
            res.json(extensions.deleteSkill(req.params.id));
        }
        catch (error) {
            next(error);
        }
