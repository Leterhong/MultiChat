...
function risk(permissions) {
    if ((permissions || []).some(item => ['filesystem', 'process', 'external'].includes(item)))
        return 'high';
...
function capabilityPassports() {
    ...
    const mcp = extensions.listMcpServers().map(item => {
        ...
        if (item.trustLevel !== 'trusted')
            issues.push('未标记为可信，调用前需要审批');
        if (item.lastError)
            issues.push(item.lastError);
    ...
    const plugins = extensions.listPlugins().map(item => {
        ...
        const issues = [];
        if (item.invalid)
            issues.push('插件结构无效或组件路径越界');
        if (!item.version || item.version === '0.0.0')
            issues.push('未声明可追踪版本');
...
function makeSnapshot(projectId, input = {}, automatic = false) {
    ...
    if (assetBytes > 7_000_000)
        throw new Error('snapshot project files exceed 7 MB; remove large files first');
    ...
    if (Buffer.byteLength(serialized, 'utf8') > 8_000_000)
        throw new Error('snapshot exceeds 8 MB; remove large project files first');
    ...
    rows.unshift(row);
    ctx.store.write(SNAPSHOT_FILE, rows.slice(0, 50));
    return row;
}
module.exports = function registerControlPlane(app) {
    app.get('/api/capabilities', (req, res, next) => { try {
        res.json(capabilityPassports());
    }
    catch (error) {
        next(error);
    } });
    app.get('/api/memories', (req, res) => {
        ...
        if (req.query.projectId)
            rows = rows.filter(item => item.projectId === req.query.projectId);
        res.json(rows);
    });
    ...
    app.post('/api/snapshots', (req, res) => {
        try {
            res.status(201).json(publicSnapshot(makeSnapshot(ctx.safeId(req.body?.projectId, 'project id'), req.body || {})));
        }
        catch (error) {
            res.status(400).json({ error: error.message });
        }
    ...
    app.post('/api/snapshots/:id/restore', (req, res) => {
        try {
            ...
            if (payload.linkedAgent) {
                ...
                if (index >= 0) {
                    agents[index] = payload.linkedAgent;
    ...
    app.delete('/api/snapshots/:id', (req, res) => {
        const rows = snapshotRows();
        if (!rows.some(item => item.id === req.params.id))
            return res.status(404).json({ error: 'snapshot not found' });
