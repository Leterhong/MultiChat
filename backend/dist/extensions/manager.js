...
function state() {
    const value = ctx.store.read(STATE_FILE, {});
    value.skills ||= {};
    value.plugins ||= {};
    value.mcp ||= {};
    return value;
...
function slug(value, label = 'id') {
    ...
    if (!result || result.length > 64)
        throw new AppError('INVALID_PACKAGE', `${label} 必须是 1-64 位小写字母、数字或连字符`);
    return result;
}
...
function parseSkillMarkdown(contents, file) {
    const text = String(contents || '').replace(/^\uFEFF/, '');
    const match = /^---\s*\r?\n([\s\S]*?)\r?\n---\s*(?:\r?\n|$)([\s\S]*)$/.exec(text);
    if (!match)
        throw new AppError('INVALID_PACKAGE', `SKILL.md 缺少 YAML frontmatter：${file}`);
    const metadata = {};
    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (/^[>|][+-]?$/.test(row[2])) {
            ...
            metadata[row[1]] = yamlScalar(row[2]);
    }
...
function skillMetadataId(metadata) {
...
function resolveMarketplaceEntry(source, entry) {
    if (!entry || typeof entry !== 'object' || !entry.name || !entry.source)
        return null;
...
function resolvePluginComponentPath(pluginDir, configured, fallback) {
    if (configured === undefined || configured === null)
        return path.join(pluginDir, fallback);
...
function mcpServerMap(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return {};
    const object = value;
    if (object.mcpServers && typeof object.mcpServers === 'object')
        return object.mcpServers;
    if (object.mcp_servers && typeof object.mcp_servers === 'object')
        return object.mcp_servers;
...
function pluginRecords() {
    ...
    for (const source of marketplaceSources()) {
        const marketplace = readJson(source.file, {});
...
function findSkill(id) {
    ...
    if (!matches.length)
        throw new AppError('NOT_FOUND', 'skill not found');
...
function createSkill(input) {
    ...
    if (!description || !instructions)
        throw new AppError('INVALID_PACKAGE', 'description 与 instructions 不能为空');
    const dir = path.join(REPO_SKILLS_ROOT, name);
    if (fs.existsSync(dir))
        throw new AppError('CONFLICT', 'skill already exists');
...
function updateSkill(id, input) {
    ...
    if (!['managed', 'repo'].includes(item.source.kind))
        throw new AppError('PERMISSION_DENIED', '此 Skill 由用户目录或插件提供，请在来源处修改');
    ...
    if (!name || !description || !instructions)
        throw new AppError('INVALID_PACKAGE', 'name、description 与 instructions 不能为空');
...
function deleteSkill(id) {
    const item = findSkill(id);
    if (!['managed', 'repo'].includes(item.source.kind))
        throw new AppError('PERMISSION_DENIED', '只能删除 MultiChat 托管或当前项目内的 Skill');
    if (!isInside(item.source.root, item.directory))
        throw new AppError('PERMISSION_DENIED', 'skill path escaped managed root');
    ...
    for (const agent of agents) {
        ...
        if (next.length !== (agent.skillIds || []).length) {
            agent.skillIds = next;
            changed = true;
        }
        if (nextRefs.length !== (agent.skillRefs || []).length) {
            agent.skillRefs = nextRefs;
            changed = true;
        }
    }
    if (changed)
...
function gitDiff(target) {
    ...
    if (rel.startsWith('..') || path.isAbsolute(rel))
        return { tracked: false, status: 'external', diff: '' };
    ...
    if (!diff && statusText.startsWith('??') && fs.existsSync(target)) {
        ...
        const walk = current => {
            for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
                if (files.length >= 80 || entry.name === 'node_modules' || entry.name === '.git')
                    continue;
                const absolute = path.join(current, entry.name);
                if (entry.isDirectory())
                    walk(absolute);
                else if (entry.isFile() && fs.statSync(absolute).size <= 256 * 1024)
                    files.push(absolute);
            }
        };
...
function pluginDiff(id) {
    ...
    if (matches.length > 1 && !matches.some(plugin => plugin.key === id))
        throw new AppError('CONFLICT', `插件 ${id} 来自多个 marketplace，请使用 source-qualified key`);
    const item = matches.find(plugin => plugin.key === id) || matches[0];
    if (!item)
        throw new AppError('NOT_FOUND', 'plugin not found');
...
function deleteImportedPlugin(id) {
    ...
    if (!marketplace || !Array.isArray(marketplace.plugins))
        throw new AppError('INVALID_PACKAGE', 'project marketplace.json is invalid');
    ...
    for (const key of Object.keys(nextState.skills)) {
        if (key.startsWith(path.resolve(item.path).toLowerCase() + path.sep))
            delete nextState.skills[key];
    }
    for (const serverId of pluginMcpIds)
        delete nextState.mcp[serverId];
    ...
    for (const agent of nextAgents) {
        ...
        if (nextSkillRefs.length !== currentSkillRefs.length) {
            agent.skillRefs = nextSkillRefs;
            agentsChanged = true;
        }
        if (nextSkillIds.length !== currentSkillIds.length) {
            agent.skillIds = nextSkillIds;
            agentsChanged = true;
        }
        if (nextMcpIds.length !== currentMcpIds.length) {
            agent.mcpServerIds = nextMcpIds;
            agentsChanged = true;
        }
    }
    ...
    try {
        ...
        saveState(nextState);
        if (agentsChanged)
            ctx.store.write(ctx.AGENT_FILE, nextAgents);
    }
    catch (error) {
        ...
        rollback('plugin package', () => {
            if (!fs.existsSync(item.path) && fs.existsSync(backup))
        ...
        if (agentsChanged)
            rollback('agent references', () => ctx.store.write(ctx.AGENT_FILE, originalAgents));
    ...
    catch { /* uninstall is already committed */ }
    toolCache.clear();
    return { ok: true, id: item.id };
}
function standaloneMcpRecords() {
    const rows = ctx.store.read(MCP_FILE, []);
    return Array.isArray(rows) ? rows : [];
}
function normalizeMcpInput(input, previous = {}) {
    ...
    if (transport === 'stdio') {
        ...
        if (!record.command)
            throw new AppError('INVALID_PACKAGE', 'STDIO server 必须提供 command');
        record.args = Array.isArray(input.args) ? input.args.map((value, index) => {
            const text = String(value);
            if ((text === '***' || /=\*\*\*$/.test(text)) && previous.args?.[index])
                return String(previous.args[index]);
            return text;
        }) : (previous.args || []);
        ...
        if (!/^https?:\/\//i.test(record.url))
            throw new AppError('INVALID_PACKAGE', 'HTTP server 必须提供 http(s) URL');
        const parsedUrl = new URL(record.url);
        if (parsedUrl.username || parsedUrl.password)
            throw new AppError('INVALID_PACKAGE', 'URL 中不能包含用户名或密码，请改用 bearer token 环境变量');
...
function expandEnvironmentMap(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return {};
    const out = {};
    for (const [key, item] of Object.entries(value)) {
        ...
        if (!expanded.missing)
            out[String(key)] = expanded.value;
    }
    return out;
}
function pluginMcpRecords() {
    const out = [];
    const currentState = state();
    for (const plugin of pluginRecords()) {
        let map = {};
        if (typeof plugin._manifest.mcpServers === 'object')
            map = mcpServerMap(plugin._manifest.mcpServers);
...
function findPrivateMcp(id) {
    ...
    if (!record)
        throw new AppError('NOT_FOUND', 'MCP server not found');
    return record;
}
function createMcpServer(input) {
    const rows = standaloneMcpRecords();
    const record = normalizeMcpInput(input);
    if (rows.some(item => item.id === record.id) || pluginMcpRecords().some(item => item.id === record.id))
        throw new AppError('CONFLICT', 'MCP server already exists');
    rows.push(record);
...
function importMcpServers(inputs, conflictPolicy = 'reject') {
    ...
    const normalized = inputs.map(input => {
        ...
        if (ids.has(record.id))
            throw new AppError('CONFLICT', `duplicate MCP server id: ${record.id}`);
        ids.add(record.id);
        if (pluginIds.has(record.id))
            throw new AppError('PERMISSION_DENIED', `MCP server ${record.id} is provided by a plugin and cannot be replaced`);
        return record;
    });
    ...
    for (const record of previous)
        ctx.closeMcpClient(mcpLaunch(record));
...
function updateMcpServer(id, input) {
    ...
    if (index < 0) {
        ...
        if (Object.keys(input).some(key => !allowed.has(key)))
            throw new AppError('PERMISSION_DENIED', '插件内 MCP server 只能修改启停、信任和作用域策略');
        const value = state();
        value.mcp[id] = {
            ...(value.mcp[id] || {}),
...
function deleteMcpServer(id) {
    ...
    if (next.length === rows.length)
        throw new AppError('PERMISSION_DENIED', '插件内 MCP server 不能单独删除');
    ctx.store.write(MCP_FILE, next);
    if (previous)
        ctx.closeMcpClient(mcpLaunch(previous));
    toolCache.delete(id);
    syncCodexMcpConfig();
    return { ok: true };
}
function mcpLaunch(record) {
    if (record.transport === 'http') {
        const headers = expandEnvironmentMap(record.headers);
        if (record.bearerTokenEnvVar && process.env[record.bearerTokenEnvVar])
            headers.Authorization = `Bearer ${process.env[record.bearerTokenEnvVar]}`;
        return {
            url: record.url,
            headers,
...
async function discoverMcpTools(id, force = false, options = {}) {
    const record = findPrivateMcp(id);
    if (!record.enabled)
        throw new AppError('PERMISSION_DENIED', 'MCP server is disabled');
    if (!force && toolCache.get(id)?.status === 'ready')
...
async function runtimeMcpTools(serverIds, options = {}) {
    ...
    const discoveredRecords = await Promise.all(records.map(async (record) => {
        try {
            return { record, discovered: await discoverMcpTools(record.id, false, options) };
        }
        catch (error) {
    ...
    for (const { record, discovered, error } of discoveredRecords) {
        if (error) {
            errors.push({ serverId: record.id, name: record.name, error: error.message });
            continue;
        }
    ...
    for (const record of standaloneMcpRecords().filter(item => item.enabled && (item.targets || []).includes('codex'))) {
        ...
        if (record.transport === 'http') {
            ...
            if (record.bearerTokenEnvVar)
                lines.push(`bearer_token_env_var = ${tomlString(record.bearerTokenEnvVar)}`);
        }
        else {
            lines.push(`command = ${tomlString(record.command)}`);
            if ((record.args || []).length)
                lines.push(`args = ${tomlArray(record.args)}`);
            if (record.cwd)
                lines.push(`cwd = ${tomlString(record.cwd)}`);
            if (Object.keys(record.env || {}).length)
                lines.push(`env_vars = ${tomlArray(Object.keys(record.env))}`);
        }
...
function migrateLegacyAgents() {
    ...
    for (const agent of agents) {
        ...
        if (!Array.isArray(agent.mcpServerIds)) {
            agent.mcpServerIds = [];
            changed = true;
        }
        if ((agent.skillIds || []).some(id => id === 'sk_mcp_time' || id === 'sk_mcp_readfile')) {
            if (!agent.mcpServerIds.includes('multichat-demo'))
                agent.mcpServerIds.push('multichat-demo');
...
function ensureDefaults() {
    ...
    const existingServers = ctx.store.read(MCP_FILE, null);
    if (!existingServers) {
    ...
    if (!ctx.store.read(STATE_FILE, null))
        saveState(state());
    migrateLegacyAgents();
    syncCodexMcpConfig();
}
