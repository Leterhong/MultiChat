...
type Props = {
  ...
  fileCount: number;
  memoryCount: number;
  execute: (target: CompareTarget, prompt: string) => Promise<CompareResult>;
  ...
  const run = async () => {
    ...
    try {
      const completed = await Promise.all(selected.map(async (target) => {
        const result = await execute(target, task);
  ...
  return <div className="compare-lab">
    ...
    <div className="compare-config">
      <fieldset className="compare-targets"><legend>参与模型</legend>{targets.map((target) => <label key={target.id}>
        <input type="checkbox" checked={selectedIds.has(target.id)} onChange={(event) => toggleTarget(target.id, event.target.checked)} />
      ...
      <div className="compare-actions"><span>已选择 {selected.length} 个模型</span><button className="btn-primary" type="button" disabled={running} onClick={() => void run()}>{running ? '实验进行中…' : results.length ? '再次实验' : '开始实验'}</button></div>
    </div>
