...
function loadHistory(): CompareRecord[] {
  try {
    ...
    return value
      .filter(
        (record) =>
          record && typeof record.id === 'string' && typeof record.prompt === 'string' && Array.isArray(record.results)
      )
      .map((record) => ({
        id: record.id,
        ...
        results: record.results.filter(
          (result: any) => result && typeof result.model === 'string' && typeof result.providerId === 'string'
        ),
      }))
      .slice(0, 12);
  } catch {
      ...
      : result.status === 'error'
        ...
        : result.status === 'cancelled'
          ...
          : result.status === 'success'
            ...
            : '等待';
  return (
    <article className="compare-result" data-state={result.status}>
      <header>
        <div>
          <strong>{result.model}</strong>
          <span>{result.providerName}</span>
        </div>
        <span className="compare-state">{stateLabel}</span>
      </header>
      <div className="compare-result-body">
        {result.status === 'running' && (
          <div className="compare-loading" aria-label="模型正在运行">
            <span />
            <span />
            <span />
          </div>
        ...
        {result.status === 'success' && (
          <div className="compare-answer">
            <SafeMarkdown>{result.text || ''}</SafeMarkdown>
          </div>
        )}
      ...
      {result.status === 'success' && (
        <footer>
          <span>{((result.elapsedMs || 0) / 1000).toFixed(1)} 秒</span>
          <span>{Number(total).toLocaleString('zh-CN')} Token</span>
          <div>
            <button
              type="button"
              onClick={async () => {
            ...
            <button type="button" className="compare-adopt" onClick={() => void adopt(result, prompt)}>
              采用到对话
            </button>
          </div>
        </footer>
...
function CompareSummary({ results }: { results: CompareResult[] }) {
  ...
  const fastest = [...successful].sort(
    (left, right) => Number(left.elapsedMs || Infinity) - Number(right.elapsedMs || Infinity)
  )[0];
  const metered = successful.filter((result) => Number(result.usage?.total_tokens ?? result.usage?.total ?? 0) > 0);
  const leanest = [...metered].sort(
    (left, right) =>
      Number(left.usage?.total_tokens ?? left.usage?.total) - Number(right.usage?.total_tokens ?? right.usage?.total)
  )[0];
  return (
    <section className="compare-summary" aria-label="实验摘要">
      <header>
        <div>
          <span>实验摘要</span>
          <strong>只比较可验证指标</strong>
        </div>
        <small>
          {successful.length} / {results.length} 个模型完成
        </small>
      </header>
      <dl>
        <div>
          ...
          <dd>
            {fastest.model}
            <small>{((fastest.elapsedMs || 0) / 1000).toFixed(1)} 秒</small>
          </dd>
        </div>
        <div>
          <dt>最低消耗</dt>
          <dd>
            ...
            <small>
              {leanest
                ? `${Number(leanest.usage?.total_tokens ?? leanest.usage?.total).toLocaleString('zh-CN')} Token`
                : '上游未返回用量'}
            </small>
          </dd>
        </div>
...
export function CompareLab({
  ...
  const run = async () => {
    ...
    try {
      ...
      if (completed.some((result) => result.status === 'success'))
        setHistory(saveHistory(task, projectName, completed));
    } finally {
  ...
  return (
    <div className="compare-lab">
      <div className="compare-intro">
        <div>
          <p>同一任务、同一上下文，并行观察不同模型的答案。</p>
        </div>
        <span className="compare-context">
          {fileCount} 个文件 · {memoryCount} 条记忆
        </span>
      </div>
      <div className="compare-config">
        <fieldset className="compare-targets">
          <legend>参与模型</legend>
          {targets.map((target) => (
            <label key={target.id}>
              <input
              ...
              <span>
                ...
                <small>{target.providerName}</small>
              </span>
            </label>
          ))}
        </fieldset>
        <label className="compare-prompt-label" htmlFor="comparePrompt">
          实验任务
        </label>
        <textarea
          id="comparePrompt"
          ...
          rows={3}
          placeholder="输入要交给多个模型的相同任务…"
          value={prompt}
          onChange={(event) => setPrompt(event.target.value)}
        />
        <div className="compare-actions">
          <span>已选择 {selected.length} 个模型</span>
          <button
            className={`btn-primary${running ? ' is-stopping' : ''}`}
            type="button"
            disabled={!running && selected.length < 2}
            onClick={() => void run()}
          >
            {running ? '停止实验' : results.length ? '再次实验' : '开始实验'}
          </button>
        </div>
      </div>
      <div className="compare-results">
        {results.map((result) => (
          <ResultCard key={result.id} result={result} prompt={prompt.trim()} adopt={adopt} />
        ))}
      </div>
      <CompareSummary results={results} />
      <details className="compare-history" open={!history.length}>
        <summary>
          <span>历史实验</span>
          <small>{history.length} 条，保存在当前设备</small>
        </summary>
        <div>
          {history.length ? (
            history.map((record) => {
              ...
              return (
                ...
                >
                  <span>
                    ...
                    <small>
                      {record.projectName} · {successCount} 个结果
                    </small>
                  </span>
                  <time>{stamp}</time>
                </button>
...
export function ModelExperimentSettings(props: Props) {
  return (
    <div className="settings-page experiment-settings">
      <div className="settings-page-heading">
        <div>
          <div className="usage-kicker">MODEL ARENA</div>
          <h3>模型竞技场</h3>
          <p>用同一个任务、参数和项目上下文并行观察结果；首页和普通对话始终只运行当前主模型。</p>
        </div>
      </div>
      {props.targets.length < 2 && (
        <div className="experiment-requirement">
          <strong>还需要一个模型</strong>
          <span>至少连接两个模型后才能进行公平的并行实验。</span>
        </div>
      )}
      <CompareLab {...props} />
    </div>
