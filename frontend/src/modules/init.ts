...
async function applyLocationRoute() {
  ...
  const compareOpen =
    $('#modal')?.classList.contains('open') && $('#modalCard')?.classList.contains('compare-modal-card');
  ...
  if (parts[0] === 'chat' && parts[1] && !isKnownConversation(parts[1])) {
...
function isKnownConversation(id: string, conversations = state.conversations) {
...
async function initApp() {
  ...
  const fb = $('#forkBtn');
  if (fb) fb.onclick = forkConversation;
...
function setupDrop() {
  ...
  zone.addEventListener('drop', async (e) => {
    ...
    if (!state.selectedProject) {
      toast('请先在右上角选择工作区/项目', 'error');
      return;
    }
    for (const file of Array.from(files) as File[]) {
      if (file.size > 2_000_000) {
        toast('文件超过 2MB 限制，已跳过：' + file.name, 'error');
        continue;
      }
      try {
        ...
        toast('文件添加失败：' + err.message, 'error');
      }
    }
    renderFileContext();
  });
...
async function forkConversation() {
  ...
  try {
    ...
    toast('分支失败：' + e.message, 'error');
  }
}

export { applyLocationRoute, bootstrap, initApp, setupDrop, forkConversation, isKnownConversation };
