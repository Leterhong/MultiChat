...
import {
  ...
  WorkspaceSettings,
  compactNumber,
} from '../components/SettingsDashboards';
...
function closeSettings() {
  ...
  window.dispatchEvent(new CustomEvent('multichat:settingschange', { detail: { open: false } }));
  if (location.hash.startsWith('#/settings/'))
    history.replaceState(null, '', state.currentConvId ? `#/chat/${encodeURIComponent(state.currentConvId)}` : '#/new');
}
...
function switchSettingsTab(tab: string) {
  ...
  renderSettings(tab);
  window.dispatchEvent(new CustomEvent('multichat:settingschange', { detail: { open: true, tab } }));
...
function renderSettings(tab = 'general', keepScroll = false) {
  ...
  if (tab === 'general') {
    mountExtensionSettings(GeneralSettings, {
      ...
      onSaveToken: (value: string) => {
        ...
        toast(value.trim() ? '访问令牌已保存' : '访问令牌已清除');
      },
    });
    ...
    mountExtensionSettings(ProviderSettings, {
      ...
      onDelete: async (provider: any) => {
        if (
          !(await showConfirm({
            title: '删除模型提供方',
            message: `删除「${provider.name || provider.id}」？此操作不可撤销。`,
            confirmLabel: '删除',
            danger: true,
          }))
        )
      ...
      onProbe: async (provider: any) =>
        api(`/api/providers/${encodeURIComponent(provider.id)}/probe`, { method: 'POST', body: JSON.stringify({}) }),
    });
  } else if (tab === 'experiment') {
    const targets = compareTargets();
    const currentId =
      state.selectedProvider && state.selectedModel ? `${state.selectedProvider.id}:${state.selectedModel}` : '';
    const defaults = [currentId, ...targets.map((item) => item.id)]
      .filter(Boolean)
      .filter((value, index, values) => values.indexOf(value) === index)
      .slice(0, 2);
    const compareDraft = localStorage.getItem('multichat_compare_draft') || '';
    const latestPrompt =
      compareDraft ||
      [...(state.messages || [])].reverse().find((message: any) => message.role === 'user')?.content ||
      '';
    if (compareDraft) localStorage.removeItem('multichat_compare_draft');
    ...
    mountExtensionSettings(SkillSettings, {
      ...
      onToggle: async (skill: any) => {
        try {
          await api(`/api/skills/${encodeURIComponent(skill.key || skill.id)}/toggle`, {
            method: 'POST',
      ...
      onDiff: async (skill: any) => {
        try {
          showDiff('Skill 变更', await api(`/api/skills/${encodeURIComponent(skill.key || skill.id)}/diff`));
        } catch (error: any) {
          toast(error.message, 'error');
        }
      },
      onDelete: async (skill: any) => {
        if (
          !(await showConfirm({
            title: '删除 Skill',
            message: '删除该 Skill？关联运行配置的引用也将被移除。',
            confirmLabel: '删除',
            danger: true,
          }))
        )
          return false;
        try {
          await api(`/api/skills/${encodeURIComponent(skill.key || skill.id)}`, { method: 'DELETE' });
    ...
    mountExtensionSettings(ToolSettings, {
      ...
      agents: state.agents,
      onTest: async (tool: any, expression?: string) =>
        api(`/api/tools/${encodeURIComponent(tool.id)}/test`, {
          method: 'POST',
          body: JSON.stringify(expression !== undefined ? { expression } : {}),
        }),
      onToggle: async (tool: any) => {
        try {
          await api(`/api/tools/${encodeURIComponent(tool.id)}`, {
            method: 'PUT',
            body: JSON.stringify({ enabled: !tool.enabled }),
          });
          await loadTools();
          toast(tool.enabled ? '工具已停用' : '工具已启用');
          return true;
        } catch (error: any) {
          toast(error.message, 'error');
          return false;
  ...
  if (keepScroll)
    requestAnimationFrame(() => {
      body.scrollTop = Math.min(previousScroll, Math.max(0, body.scrollHeight - body.clientHeight));
    });
}

function mountWorkspaceSettings() {
  mountExtensionSettings(WorkspaceSettings, {
    onProjectChange: async (id: string) => {
      state.selectedProject = state.projects.find((item: any) => item.id === id) || null;
    ...
    onSaveDefaults: async (agentId: string | null, providerId: string | null, model: string | null) => {
      ...
      try {
        await api(`/api/projects/${encodeURIComponent(state.selectedProject.id)}`, {
          method: 'PUT',
          body: JSON.stringify({ defaultAgentId: agentId, defaultProviderId: providerId, defaultModel: model }),
        });
        const updated = {
    ...
    onSearch: async (query: string) =>
      state.selectedProject
        ? api(`/api/projects/${encodeURIComponent(state.selectedProject.id)}/search?q=${encodeURIComponent(query)}`)
        : [],
    onAddMemory: () => showMemoryModal(),
    onEditMemory: (id: string) => showMemoryModal(id),
    onToggleMemory: async (item: any) => {
    ...
    onCreateSnapshot: async () => {
      ...
      const title = await showPrompt({
        title: '新建项目快照',
        label: '快照名称',
        value: `项目快照 ${new Date().toLocaleString('zh-CN')}`,
        maxLength: 120,
      });
      ...
      try {
        await api('/api/snapshots', {
          method: 'POST',
          body: JSON.stringify({ projectId: state.selectedProject.id, title }),
        });
        await loadProjectControlData();
        toast('项目快照已创建');
      } catch (error: any) {
    ...
    onRestoreSnapshot: async (id: string) => {
      if (
        !(await showConfirm({
          title: '恢复快照',
          message: '恢复该快照？当前状态会先自动备份，随后替换项目文件、记忆和默认配置。',
          confirmLabel: '恢复快照',
          danger: true,
        }))
      )
        return;
      try {
        await api(`/api/snapshots/${encodeURIComponent(id)}/restore`, { method: 'POST' });
    ...
    onDeleteSnapshot: async (id: string) => {
      if (
        !(await showConfirm({ title: '删除项目快照', message: '删除该项目快照？', confirmLabel: '删除', danger: true }))
      )
        return;
...
function mountAgentSettings() {
  mountExtensionSettings(AgentSettings, {
    ...
    onDelete: async (agent: any) => {
      if (
        !(await showConfirm({ title: '删除运行配置', message: '删除该运行配置？', confirmLabel: '删除', danger: true }))
      )
        return;
      try {
    ...
    onImport: async (file: File) => {
      try {
        await doImport(normalizeImport(JSON.parse(await file.text())), `文件 ${file.name}`);
      } catch (error: any) {
...
function exportUsage(usage: any, range: string) {
  const rows = [
    ['date', 'input_tokens', 'output_tokens', 'total_tokens', 'requests', 'errors'],
    ...(usage?.daily || []).map((item: any) => [
      item.date,
      item.inputTokens,
      item.outputTokens,
      item.totalTokens,
  ...
  link.download = `multichat-usage-${range}d.csv`;
  link.click();
...
function showWorkspaceForm() {
  showModal({
    ...
    onMount: (card: HTMLElement) => {
      ...
      $('#workspaceForm', card).onsubmit = async (event: SubmitEvent) => {
        event.preventDefault();
...
function showMemoryModal(id: string | null = null) {
  ...
  showModal({
    ...
    onMount: (card: HTMLElement) => {
      ...
      $('#memoryForm', card).onsubmit = async (event: SubmitEvent) => {
        event.preventDefault();
        ...
        try {
          await api(editing ? `/api/memories/${encodeURIComponent(editing.id)}` : '/api/memories', {
          ...
          $('#memoryErr', card).textContent = error.message;
        }
      };
    },
  });
...
function showProjectForm() {
  ...
  showModal({
    ...
    onMount: (card: HTMLElement) => {
      ...
      $('#projectForm', card).onsubmit = async (event: SubmitEvent) => {
        event.preventDefault();
        const data: any = Object.fromEntries(new FormData(event.target as HTMLFormElement).entries());
        data.workspaceId = state.selectedWorkspace.id;
        try {
...
function showAssetUrlModal() {
  ...
  showModal({
    ...
    onMount: (card: HTMLElement) => {
      ...
      $('#assetUrlForm', card).onsubmit = async (event: SubmitEvent) => {
        event.preventDefault();
        const data: any = Object.fromEntries(new FormData(event.target as HTMLFormElement).entries());
        data.projectId = state.selectedProject.id;
        try {
          await api('/api/assets', { method: 'POST', body: JSON.stringify(data) });
          await loadProjects();
          closeModal();
          toast('远程文件已加入项目');
        } catch (error: any) {
          $('#assetUrlErr', card).textContent = error.message;
        }
      };
    },
  });
  ...
  try {
    ...
    showModal({
      ...
      body: `<div class="run-detail-summary">${chip('状态', statusText)}${chip('模型', run.model || '—')}${chip('Token', compactNumber(usage.totalTokens || 0))}${chip('步骤', String(steps.length))}${chip('策略', run.toolPolicy === 'safe' ? '安全' : '自动')}</div><div class="run-detail-grid"><section><h4>执行时间线</h4><div class="flight-timeline">${steps.map((step: any, index: number) => `<article class="flight-step ${esc(step.status)}"><i>${index + 1}</i><div><strong>${step.kind === 'tool_call' ? esc(step.tool || '工具调用') : `模型请求 · ${esc(step.model || '')}`}</strong><p>${step.kind === 'tool_call' ? `${esc((step.permissions || []).join(', ') || '无特殊权限')} · ${esc(step.risk || 'low')}` : `${step.messageCount || 0} 条上下文 · ${compactNumber(step.usage?.total || 0)} tokens`}</p>${step.error ? `<code>${esc(step.error)}</code>` : ''}</div><span>${step.durationMs != null ? `${(step.durationMs / 1000).toFixed(2)}s` : esc(step.status)}</span></article>`).join('') || '<div class="control-empty">没有步骤轨迹。</div>'}</div></section><section><h4>上下文透镜</h4><div class="context-lens"><div><span>消息</span><strong>${context.messages || 0}</strong></div><div><span>系统上下文</span><strong>${compactNumber(context.systemCharacters || 0)} 字</strong></div><div><span>项目文件</span><strong>${(context.assets || []).length}</strong></div><div><span>记忆</span><strong>${(context.memories || []).length}</strong></div></div><div class="context-source"><h5>注入来源</h5>${[
        ...
        ['工具', context.tools],
        ['MCP', context.mcpServers],
      ]
        .map(
          ([label, items]: any) =>
            `<div><strong>${label}</strong><span>${(items || []).map((item: any) => esc(item.name || item.id)).join('、') || '无'}</span></div>`
        )
        .join(
          ''
