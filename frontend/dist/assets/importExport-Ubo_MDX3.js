import{S as e,c as t,g as n,w as r,x as i}from"./index-fl2LpQUK.js";function a(){return`
    <div class="import-bar">
      <span class="import-bar-label">运行配置备份</span>
      <button class="mini-btn" id="importFile">导入配置 JSON</button>
      <input type="file" id="importFileInput" accept=".json,application/json" style="display:none" />
    </div>`}async function o(e,r){try{let a=await t(`/api/import`,{method:`POST`,body:JSON.stringify(e)});return await loadAgents(),renderSettings(n.currentTab||`agents`,!0),renderTopbar(),i(`已导入 ${a.agents} 个运行配置（来源：${r}）`),a}catch(e){throw i(e.message,`error`),e}}function s(e){if(Array.isArray(e))return{agents:e};if(e&&e.agents)return{agents:e.agents};if(e&&typeof e==`object`&&e.id)return{agents:[e]};throw Error(`文件不是 Agent JSON 备份`)}function c(){let t=e(`#settingsBody`),n=t.querySelector(`#importFile`),r=t.querySelector(`#importFileInput`);n&&r&&(n.onclick=()=>r.click(),r.onchange=async()=>{let e=r.files&&r.files[0];if(e){try{let t=await e.text();await o(s(JSON.parse(t)),`文件 `+e.name)}catch(e){i(`解析失败：`+e.message,`error`)}r.value=``}})}function l(e,t){let n=new Blob([JSON.stringify(e,null,2)],{type:`application/json`}),r=document.createElement(`a`);r.href=URL.createObjectURL(n),r.download=(e.id||t)+`.json`,document.body.appendChild(r),r.click(),r.remove(),setTimeout(()=>URL.revokeObjectURL(r.href),1e3),i(`已导出：`+(e.name||e.id))}function u(a){let o=!!a,s=o?n.agents.find(e=>e.id===a):null;showModal({title:o?`编辑运行配置`:`新建运行配置`,body:`<form id="agentForm">
      ...
      <div class="field"><label>Agent Skills（先提供描述，匹配任务时再加载完整工作流）</label>
        <div class="extension-checks">
          ${n.skills.map(e=>{let t=e.key||e.id,n=s&&(s.skillRefs||s.skillIds)||[],i=n.includes(t)||n.includes(e.id)?`checked`:``;return`<label>
      ...
      <div class="field"><label>内置工具（函数调用）</label>
        <div class="extension-checks">
          ${(n.tools||[]).map(e=>{let t=s&&(s.toolIds||[]).includes(e.id)?`checked`:``;return`<label><input type="checkbox" name="tool" value="${r(e.id)}" ${t} /> ${r(e.name)}${e.enabled?``:`（已停用）`}</label>`}).join(``)||`<span class="pmeta">没有已启用的内置工具</span>`}
        </div>
      </div>
      <div class="field"><label>MCP servers（工具由 tools/list 实时发现）</label>
        <div class="extension-checks">
          ${(n.mcpServers||[]).filter(e=>(e.targets||[`multichat`]).includes(`multichat`)).map(e=>{let t=s&&(s.mcpServerIds||[]).includes(e.id)?`checked`:``;return`<label><input type="checkbox" name="mcpServer" value="${r(e.id)}" ${t} /> ${r(e.name)} <span class="pmeta">${(e.tools||[]).length} tools${e.enabled?``:` · 已停用`}</span></label>`}).join(``)||`<span class="pmeta">没有已启用的 MCP server</span>`}
        </div>
      </div>
