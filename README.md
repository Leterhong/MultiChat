...
- **开发任务闭环**：每个项目和对话拥有独立的执行、规划、审查模式；任务步骤、验收标准与测试、复查、安全检查会形成隐藏工作协议，不污染用户消息正文；
- **三栏开发工作台**：左侧管理对话，中间专注任务与结果，右侧在运行、计划和活动三个分区之间切换；窄屏下自动变为可关闭的任务抽屉；
- **零配置体验**：通过“本地体验（无需密钥）”走通模型选择、流式对话、运行记录与 Token 用量；
- **并行模型实验**：让 2–4 个模型在相同参数、项目文件和记忆上下文下并行回答；可以随时停止请求，并以响应时间、Token 消耗和上下文一致性这些可验证证据生成摘要，再把选定结果采用到正式对话；
...
- **执行与复盘**：保留模型步骤、工具调用、审批、错误与 Token 构成，并在活动分区汇总可验证的运行证据；
...
frontend/src/core/  客户端状态、任务工作协议、API、主题与基础工具
...
| Plugin | 分发一组完整扩展资源 | `plugin.json` 清单与组件目录 |
